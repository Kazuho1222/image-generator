...
function App() {
  ...
  async function fetchImages() {
    const { data, error } = await supabase.storage.from("generate-image").list();
    ...
    if (data) {
      const imageUrls = await Promise.all(
        data.map(async (image) => {
          ...
          const { data: signedUrData, error: signedUrlError } = await supabase.storage.from("generate-image").createSignedUrl(image.name, 60);
  ...
  const handleSaveImage = async () => {
    ...
    // 画像をストレージにアップロード
    const { error } = await supabase.storage.from("generate-image").upload(fileName, binaryData.buffer, {
  ...
  return (
    <div className='mi-h-screen bg-gradient-to-br from-black via-gray-900 to-gray-800 text-white p-8'>
      ...
      <div className='flex justify-center mb-8'>
        <div className='flex flex-col sm:flex-row gap-2 p-4 bg-gray-800 rounded-lg shadow-lg w-full max-w-3xl'>
          <input type="text" className='flex-grow p-3 bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 transition' placeholder='Describe your imagination...' value={prompt} onChange={(e) => setPrompt(e.target.value)} />
          <button
            disabled={isLoading}
            className='bg-gradient-to-r from-purple-500 text-white px-6 py-3 rounded-md hover:opacity-80 transition-opacity disabled:opacity-50 flex items-center justify-center'
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 max-w-xl mx-auto'>
        {imageList.map((imageUrl, index) => (
          <div
            key={imageUrl}
            className='relative group aspect-square overflow-hidden rounded-lg shadow-xl transition-all duration-300 hover:scale-105'
          >
            <img
              src={imageUrl}
              alt={`Generated ${index}`}
